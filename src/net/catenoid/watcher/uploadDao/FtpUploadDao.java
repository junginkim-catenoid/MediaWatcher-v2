...
//			log.debug(sql);
            ...
            while (results.next()) {
                ...
                if (nMaxCount > 0 && nCount >= nMaxCount) {
                    log.info(String.format("We will process only %d items, because large request can occur error.", nMaxCount));
                    break;
                }
            }
    ...
    protected ArrayList<FileItemDTO> selectCompareOver(String chk_time) throws Exception {
        ...
        try {
            ...
            while (results.next()) {
                ...
                log.debug("Select work file: " + item.toString());

                if (count >= max) {
                    log.info(String.format("We will process only %d items, because large request can occur error. (create snapshot)", max));
                    break;
                }
            }
