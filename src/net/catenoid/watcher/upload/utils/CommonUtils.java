...
//                : conf.get_kollus_api().get_url(ModuleConfig.URLS.WATCHER_LIST_COMPLETE);
        ...
        try {
            ...
            if (response == null) {
                ...
                if (nStatus != 200) {
                    transmitErrorReport(nStatus, ModuleConfig.URLS.WATCHER_LIST_DELETE, "", conf);
    ...
    private List<NameValuePair> getHttpQueryParam(SendFileItemsDTO sendItem, String type) throws Exception {
        ...
        switch (type) {
            case "register":
                ...
                qparams.add(new BasicNameValuePair(Role_Watcher.PARAM.WATCHER_FILES, jsonStr));
                break;
            case "copy":
                qparams.add(new BasicNameValuePair(Role_Watcher.PARAM.API_KEY, info.getApiKey()));
                qparams.add(new BasicNameValuePair(Role_Watcher.PARAM.API_REFERENCE, info.getApiReference()));
        ...
        log.trace("getHttpQueryParam: " + jsonStr);
...
//        }
    ...
    protected boolean fileNamePatternCheck(String fileName) {
        ...
        // [MediaWatcher2/issues/5]
        // 확장자가 꼭3문자는 아니다. 1문자 이상으로 수정.
        String consoleFilePattern = "^\\d{8}-\\w{8}.\\w+$";
